...
default_args = {
  ...
  'raw_data_topic' : 'rtms-raw-preprocess', # *************** INCLUDE ONLY ONE TOPIC - This is one of the topic you created in SYSTEM STEP 2
  ...
  'rtmsstream' : 'rtms-log-data', # Change as needed - STREAM containing log file data (or other data) for RTMS
                                                    # If entitystream is empty, TML uses the preprocess type only.
  'identifier' : 'RTMS Past Memory of Events', # <<< ** Change as needed
  'searchterms' : '--entity-- password failure ~~~ |unknown error --entity--', # main Search terms, if AND add @, if OR use | s first characters, default OR
