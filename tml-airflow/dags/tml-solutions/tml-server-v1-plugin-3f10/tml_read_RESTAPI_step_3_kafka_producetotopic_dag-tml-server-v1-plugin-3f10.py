 ...
 #               return
              ...
              ######################## PUT IN LOOP #########################################
                ...
                with sim_lock:  # ✅ Thread-safe
                  ...
                  sg.active_sim_threads.append(sg.sim_thread)                  
                  sg.sim_thread.start()
             ...
             #   if sg.read_job and sg.read_job["stop"]:
              ...
              #      pass
                ...
                if sg.read_thread and sg.read_thread.is_alive():
                    ...
                    sg.read_thread.join(timeout=3.0)
...
#                startreg = req.get("start_register", 40001),
                ...
                sg.active_read_threads.append(sg.read_thread)
...
#            print(json.dumps(data))
        ...
        def stop_vessel_read():
            if sg.read_thread.is_alive():
                    # 1. Set the Event Signal first!
                  sg.stop_read.set()
                    
            # 2. Update the job dict for record-keeping
            if sg.read_job:
                sg.read_job["stop"] = True
                        
                    # 3. Wait for the thread to actually exit
            for t in sg.active_read_threads:
                    if t.is_alive():
                        t.join(timeout=2.0)

            sg.active_read_threads = [] 
#            sg.read_thread.join(timeout=2.0)
  
            return {"message": "Stop signal sent"}

        @app.post("/api/v1/scada_read_carryover_stop")
        def stop_vessel_carryover_read():
           if sg.sim_thread.is_alive():
             # 1. Signal the physics engine to stop
             sg.stop_sim.set()
        
        # 2. Update the job dict
           if sg.sim_job:
              sg.sim_job["stop"] = True            
        # 3. Wait for the Numba/Physics loop to finish its current step
           for t in sg.active_sim_threads:
                 if t.is_alive():
                        t.join(timeout=2.0)

           sg.active_sim_threads = []
#           sg.sim_thread.join(timeout=2.0)
