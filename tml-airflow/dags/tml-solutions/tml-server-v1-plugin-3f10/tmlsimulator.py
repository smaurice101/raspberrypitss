...
def build_complete_topology(scada_json, base_topology):
    """
    SUBSET validation: required_vessels <= vessel_physics.keys()
    """
    # Extract vessel physics
   # print("scada_json==========",scada_json)
    ...
    # DYNAMIC required vessels from topology
    required_vessels = {vessel['id'] for vessel in base_topology['vessels']}

  #  print("required_vessels=",required_vessels)
    
    # FIXED: SUBSET CHECK - required ⊆ available
    missing = required_vessels - set(vessel_physics.keys())

   # print("vessel_physics.keys()===",vessel_physics.keys())
  
    print('mising==',missing)
  
    if missing:
        print(f"MISSING vessels: {missing} (have {len(vessel_physics)} vessels)")
        return None
    
    # ✅ All required vessels found → build topology
    updated_vessels = []
    for vessel in base_topology['vessels']:
        vid = vessel['id']
        physics = vessel_physics[vid]
    ...
    for flow in base_topology['flows']:
        ...
        updated_flows.append({
            "from": from_id,
            "to": flow['to'],
            "F_in": vessel_physics[from_id]["F_in"],
            "f_gas": vessel_physics[from_id]["f_gas"]
        })
...
def physics_carryover(
    ...
    """
    Final Production Build: Topology-Agnostic with Anti-Ramp Stabilization.
    """
    nv = len(y0)
    ...
    # 1. PROCESS DYNAMICS & SENSOR EMULATION
    # Kp/Ki tuned for overdamped response; 'alpha' is the noise-smoothing factor
    Kp, Ki, alpha = 0.45, 0.01, 0.15
    integral_err = np.zeros(nv)
    setpoints = y0 * 1.02 # Setpoint 2% above initial volume
    
    # Static Quadrature for Droplet Physics
    ZVALS = np.array([-1.2, 0.0, 1.2])
    WEIGHTS = np.array([0.25, 0.5, 0.25])

    for t in range(1, t_steps):
        # 2. STOCHASTIC TURBULENCE GENERATION
        # Mimics the 1-2% flow variation in real separators
        ...
        for i in range(nv):
            # --- NUMERICAL FORTRESS (DivByZero Protection) ---
            s_area, s_ht = max(areas[i], 1e-6), max(hts[i], 1e-3)
            s_rhog, s_mug = max(rhog[i], 1e-6), max(mug[i], 1e-9)
            
            # --- DYNAMIC HEADSPACE ---
            v_curr = v_state[t-1, i]
            level_f = min((v_curr / s_area) / s_ht, 0.98)
            # Gas area shrinks non-linearly with level
            gas_mod = max((1.0 - level_f)**3.0, 1e-7)
            
            q_g_m3s = fg_in[i] / 3600.0
            v_g = q_g_m3s / (s_area * gas_mod)

            # --- DROPLET SETTLING PHYSICS ---
            v_s = vs_souders[i] * np.sqrt(abs(rhol[i] - s_rhog) / s_rhog)
            loading = max(min(v_g / max(v_s, 1e-6), 4.0), 0.01)
            
            lmu, lst = np.log(max(dmean[i], 1e-1)), np.log(max(dsig[i], 1.01))
            cf_class_sum = 0.0
            for j in range(3):
                dm = np.exp(lmu + ZVALS[j] * lst) * 1e-6
                v_stokes = (rhol[i] - s_rhog) * g * (dm**2) / (18.0 * s_mug)
                stn = (v_stokes * gh[i]) / (v_g * s_ht + 1e-10)
                e_s = 1.0 - np.exp(-max(min(stn, 20.0), 0.0))
                et = 1.0 - (1.0 - ein[i]) * (1.0 - e_s) * (em_input[i] * loading)
                cf_class_sum += (1.0 - et) * WEIGHTS[j]

            # 3. ANTI-RAMP STABILIZATION & FLICKER
            cf_inst = max(cf_class_sum * carryover_scale[i], 1e-6)
            
            # Stochastic flicker (Sensor Jitter)
            flicker = 1.0 + (np.random.standard_normal() * 0.015)
            carryover[t, i] = cf_inst * flicker

            # --- DYNAMIC VALVE RESPONSE (LCV) ---
            err = v_curr - setpoints[i]
            # Leaky Integral prevents infinite accumulation/ramping
            ...
            # Final Mass Balance Integration
...
def physics_carryover2(
    ...
    """
    Final Production Build: Topology-Agnostic with Enhanced Numerical Fortress.
    Protects against DivByZero and NaN propagation in high-speed streaming environments.
    """
    nv = len(y0)
    carryover = np.zeros((t_steps, nv))
    v_state = np.zeros((t_steps, nv))
    ...
    for t in range(1, t_steps):
        ...
        for i in range(nv):
            # --- ENHANCED NUMERICAL FORTRESS (DivByZero & NaN Protection) ---
            s_area = max(areas[i], 1e-6)
            s_ht = max(hts[i], 1e-3)
            s_rhog = max(rhog[i], 1e-6)
            s_rhol = max(rhol[i], 1.0)     # Critical: Protect liquid density
            s_mug = max(mug[i], 1e-9)
            
            # --- DYNAMIC HEADSPACE ---
            v_curr = v_state[t-1, i]
            # Ensure level_f doesn't create a zero headspace (max 99% full)
            level_f = min(max((v_curr / s_area) / s_ht, 0.0), 0.99)
            # Gas area shrinks non-linearly with level
            gas_mod = max((1.0 - level_f)**3.0, 1e-7)
            
            q_g_m3s = fg_in[i] / 3600.0
            # Protect gas velocity calculation from extreme headspace contraction
            v_g = q_g_m3s / max(s_area * gas_mod, 1e-10)

            # --- DROPLET SETTLING PHYSICS ---
            # Protect sqrt from density parity (rhol - rhog) and zero rhog
            v_s_base = max(abs(s_rhol - s_rhog) / s_rhog, 1e-9)
            v_s = vs_souders[i] * np.sqrt(v_s_base)
            
            # Loading Factor: Protect against v_s being 0
            loading = max(min(v_g / max(v_s, 1e-6), 4.0), 0.01)
            
            # Droplet size distribution log-space mapping
            lmu = np.log(max(dmean[i], 1e-1))
            lst = np.log(max(dsig[i], 1.01))
            
            cf_class_sum = 0.0
            for j in range(3):
                # Droplet diameter integration (Quadrature)
                dm = np.exp(lmu + ZVALS[j] * lst) * 1e-6
                v_stokes = (s_rhol - s_rhog) * g * (dm**2) / (18.0 * s_mug)
                
                # Separation Efficiency Stage-Gate
                stn = (v_stokes * gh[i]) / (v_g * s_ht + 1e-10)
                e_s = 1.0 - np.exp(-max(min(stn, 20.0), 0.0))
                
                # Combined efficiency across stages
                et = 1.0 - (1.0 - ein[i]) * (1.0 - e_s) * (em_input[i] * loading)
                cf_class_sum += (1.0 - et) * WEIGHTS[j]
            ...
            # Stochastic flicker (Sensor Jitter emulation)
            flicker = 1.0 + (np.random.standard_normal() * 0.015)
            carryover[t, i] = cf_inst * flicker

            # --- DYNAMIC VALVE RESPONSE (LCV Control Logic) ---
            err = v_curr - setpoints[i]
            # Leaky Integral prevents infinite error accumulation/ramping
            integral_err[i] = (integral_err[i] * 0.98) + (err * dt)
            
            q_in = fl_in[i] / s_rhol
            # Balanced flow control simulation
            q_out = max(Kp * err + Ki * integral_err[i] + q_in, 0.0)
            
            # Final Mass Balance Integration (Euler Method)
            v_state[t, i] = max(v_curr + (q_in - q_out - (cf_inst * q_in)) * dt, 0.0)

    return carryover

@njit(fastmath=True)
def physics_carryover3(
    t_steps, dt, y0, 
    f_gas_mat, f_liq_mat,
    areas, hts, rhol, rhog, mug,
    ein, em_input, gh, vs_souders, dmean, dsig,
...
class PhysicsCarryover:
    def __init__(self, cfg, warm_up=True):
        self.cfg = cfg
...
def runsim(jsonstr,filename,dt=0.1,modestr="carryover_snapshot"):
       global simglobal
  ...
  #     try:
       ...
       if simglobal is None or id(simglobal.cfg) != id(cfg):
            simglobal = PhysicsCarryover(cfg)
       else:
...
def simloop(scadarawdatatopic,
        ...
        while not sg.sim_job['stop']: #True:
          try:
         ...
         #      modestr = ""
            ...
            if response.text != "":
              ...
              if updated_topology:
                  ...
                  jsonsim = runsim(json.dumps(updated_topology),filename,mdt,modestr)
