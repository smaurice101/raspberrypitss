...
default_args = {
 ...
 'agents_topic_prompt' : """
        iot-preprocess:These real-time data are from IoT devices, they are being processed by TML.  There are several variables being processed 
             from each device such as voltage, current, power, engergy used.  I want you to extract insights from the data that show decrease or
             increase in the hyperprediction field.  The type of processing being done is in the processtype field.  The value of the processing
             is in the hyperprediction field.  Indicate the device data being processed, its name is in the mainuid field. For example, given this data
             [hyperprediction=122601.750, processtype=avg, processvariable=voltage, mainuid=ac000w016399359], it says for processvariable=voltage,
             and device name mainuid=ac000w016399359, the processtype avg or average, the avg value for voltage is 122601.758.  
             Do your analysis by grouping on the mainuid field and determine if there are any issues or anaomlies in the IoT devices?;
        iot-ml-prediction-results-output:These are real-time machine learning IoT device failure probability predictions.  I want you to 
             analyse the hyperprediction field to determine if the failure probability predictions for devices exceed 0.80.  If it does exceed,
             then this means the device is likely to fail, and you should flag this device for investigation. For example, given this data,
             [hyperprediction=0.75, preprocesstype=probability prediction, mainuid=ac000w016399378], it says that device name mainuid=ac000w016399378
             has a failure probability (processtype=probability prediction) of 75% (hyperprediction=0.75), so this device should NOT be investigated. But,
             if [hyperprediction=0.95, processtype=probability prediction, mainuid=ac000w0163371], says the device name mainuid=ac000w0163371 has
             a 95% (hyperprediction=0.95) of failing and so this device must be investigated.  You must flag the devices (in the mainuid field) that have a 
             high probability of failing greater than 0.80.
""", # <topic agent will monitor:prompt you want for the agent>
 ...
 'teamleadprompt' : """
      ...
      highlight the devices name (i.e. in the mainuid field) with details on whether the device failure probabilities are increasing or greater than 0.80.
...
""", # Enter the team lead prompt 
 ...
 'supervisorprompt' : """
       ...
       If you find a URGENT concern in the failure probabilities of IoT devices indicated by the team lead agent, 
       then use the send_email tool to send an email message that 
       highlights devices with the issues that need investigation. Only send an email if its an URGENT issue. Do not write python script
       to analyse the data.
